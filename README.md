...
In this guide, you will learn how to create a marketplace like [OpenSea](https://opensea.io/) on the ArbitrumTestnet Ethereum test network!
...
```tsx
<ThirdwebProvider desiredChainId={ChainId.ArbitrumTestnet}>
  <Component {...pageProps} />
</ThirdwebProvider>
```
...
async function createAuctionListing(
  ...
  try {
    const transaction = await marketplace?.auction.createListing({
      ...
      currencyContractAddress: NATIVE_TOKEN_ADDRESS, // NATIVE_TOKEN_ADDRESS is the cryptocurency that is native to the network. i.e. ArbitrumTestnet Ether
...
**Note:** For making offers, you'll need to have an ERC20 token. For our ArbitrumTestnet marketplace, that means you'll need to have wrapped ETH (wETH).
